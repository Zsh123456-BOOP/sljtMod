...
- `probe.log`
- `mods\Sts2Mcp\probe_mod.log`（便于直接从游戏目录抓排障日志）
...
- `Analytics`
  ...
  - `CurrentContributionBoard` / `TotalContributionBoard`：团队贡献榜（rDPS风格拆分）
...
- `战斗`：默认显示“当前战斗综合贡献榜”（直伤/辅助增伤/减伤/团队护盾/状态分），可切换到文本详情
- `战斗`：榜单每行带占比柱状图
- `总计`：累计统计、单机/联机拆分、卡牌使用率 Top
- `路线`：当前分叉评分、建议路线、风险标签
- `路线`：可点击路线选项，在地图界面高亮整条路线
- `路线`：内置图例类型按钮（小怪/精英/问号/商店/休息/宝箱/古遗迹/首领），悬停预览、点击锁定该类型最佳路线
- `路线`：额外显示历史统计榜（最多/最优/最亏）与最近路径
- `日志`：最近战斗事件流（可快速复盘）
- 交互：拖拽标题栏移动，右下角拖拽缩放窗口，`A-` / `A+` 调透明，`F8` 显示/隐藏，`F7` 重置并显示，战斗页可 `榜单/详情` 切换
...
存档说明：
- 安装器只改 `mods\Sts2Mcp\`，不会读写游戏存档目录。
- 若出现“像丢档”的情况，优先检查：
- Steam 云存档是否同步到了其他机器/账号。
- 游戏是否切到了不同存档槽位或分支。
- 是否曾手动清理过 `%USERPROFILE%\AppData\LocalLow` / `%APPDATA%` 下的游戏目录（具体路径以游戏实际实现为准）。
